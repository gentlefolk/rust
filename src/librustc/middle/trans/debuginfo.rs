...
{
    // This function indicates whether a function is local to the current compilation
    ...
    // (by being externally visible or by being inlined into something externally visible). It
    // might better to use the `exported_items` set from `driver::CrateAnalysis` in the future,
    // but (atm) this set is not available in the translation pass.
    let reachable = cx.reachable.borrow();
    !reachable.get().contains(&node_id)
}
